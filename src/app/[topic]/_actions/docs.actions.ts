...
export async function getDocPage(topicSlug: string, slugPath: string, locale?: string) {
...
// ─── GET /docs/page/:topicSlug/* (client-side, no cache) ─────────────────────

export async function getDocPageClient(topicSlug: string, slugPath: string, locale?: string) {
  const url = new URL(`${API_URL}/docs/page/${topicSlug}/${slugPath}`)
  if (locale && locale !== 'en') url.searchParams.set('locale', locale)
...
export async function triggerTranslation(nodeId: string, locale: string) {
  return apiFetch<{ cached: boolean; locale: string }>(`${API_URL}/docs/nodes/${nodeId}/translate`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export function buildSidebarNavData(tree: DocTree, topicSlug: string): SidebarNavData {
